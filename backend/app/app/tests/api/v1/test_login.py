...
def test_use_superuser_access_token(
    ...
    r = client.post(
        settings.TOKEN_TEST_URL, headers=superuser_token_headers,
    )
    result = r.json()
    assert r.status_code == 200
...
def test_use_user_access_token(
    ...
    r = client.post(
        settings.TOKEN_TEST_URL, headers=normal_user_token_headers,
    )
    result = r.json()
    assert r.status_code == 200
    assert "email" in result
...
def custom_user_token_headers(
    ...
    return get_custom_user_token_headers(client, settings, collection, scopes=['role:custom'])
